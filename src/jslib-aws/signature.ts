...
export class SignatureV4 {
  ...
  sign(
    ...
    if (ArrayBuffer.isView(request.body)) {
      request.body = request.body.buffer as ArrayBuffer;
    }
    ...
    if (!this.signature || !this.stringToSign) {
      throw new InvalidSignatureError(
        "Failed to compute signature or string to sign",
      );
    }
  ...
  private calculateSignature(
    ...
    this.signature = crypto.hmac("sha256", (signingKey as unknown) as ArrayBuffer, stringToSign, "hex");
  ...
  private deriveSigningKey(
    ...
    const kDateBytes: any = crypto.hmac(
      "sha256",
      "AWS4" + kSecret,
      shortDate,
      "binary",
    ...
    const kRegionBytes: any = crypto.hmac(
      "sha256",
    ...
    const kServiceBytes: any = crypto.hmac(
      "sha256",
    ...
    const kSigningBytes: any = crypto.hmac(
      "sha256",
  ...
  private computePayloadHash({ headers, body }: HTTPRequest): string {
    ...
    if (ArrayBuffer.isView(body)) {
      // If the request body is a typed array, we need to convert it to a buffer
      // so that we can calculate the checksum.
      return crypto.sha256((body as DataView).buffer as ArrayBuffer, "hex").toLowerCase();
    }
 ...
 */
function bytesToUint8Array(bytes: any): Uint8Array {
